...
export default function JobPage({ params }: JobPageProps) {
  ...
  /* ------------------------------------------------------------------------ */
  /* Load Data with Page support                                              */
  /* ------------------------------------------------------------------------ */

  const loadData = useCallback(
    async (pageToLoad = currentPage, showRefresh = false) => {
      if (showRefresh) {
        setRefreshing(true);
      }
      ...
      try {
        const [jobRes, execRes] = await Promise.all([
          ...
          fetch(`/api/executions?jobId=${jobId}&page=${pageToLoad}&limit=10`),
        ]);
  ...
  const handleRunNow = async () => {
    ...
    try {
      ...
      // پس از اجرای جدید، به صفحه ۱ برمی‌گردیم
      setCurrentPage(1);
      await loadData(1);
    } catch (err) {
      console.error(err);
  ...
  const handleCopyUrl = async () => {
    if (!job?.parsed_request?.url) return;
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="flex items-center gap-3 text-neutral-500">
          ...
          <span className="text-xs tracking-[0.25em]">LOADING JOB CONSOLE...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto w-full mt-10">
        <div className="border border-neutral-800 border-dashed bg-neutral-900/50 py-16 text-center">
          ...
          <p className="text-xs tracking-[0.25em] text-neutral-500">JOB NOT FOUND</p>
          <Link
            href="/"
  ...
  const totalExecutions = meta?.total ?? executions.length;
    ...
    <div className="max-w-6xl mx-auto w-full px-4 sm:px-6 py-8 space-y-6">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-neutral-900 border border-neutral-800 p-4">
          ...
          <div className="text-[10px] text-neutral-600 mt-1">all time</div>
        </div>
        ...
        <div className="bg-neutral-900 border border-neutral-800 p-4">
          ...
          <div className="text-[10px] text-neutral-600 mt-1">
            {successfulExecutions} successful on this page
          </div>
        </div>
        ...
        <div className="bg-neutral-900 border border-neutral-800 p-4">
          ...
          <div className="text-[10px] text-neutral-600 mt-1">on this page</div>
        </div>
          ...
          <section className="bg-neutral-900 border border-neutral-800">
            ...
            <div className="p-4">
              <div className="flex items-center gap-2 overflow-x-auto pb-1">
                {[
                  { label: "TODAY", value: job.last_success_date || "CURRENT" },
                  { label: "+1 DAY", value: "NEXT TARGET" },
                  { label: "+2 DAYS", value: "NEXT TARGET" },
                ].map((item, index) => (
              ...
              <div className="mt-4 flex items-center gap-2 text-[10px] text-neutral-600">
                ...
                <span>
                  Handles retries per target date. Skips already-booked days automatically.
                </span>
              </div>
            </div>
          </section>
          ...
          <section className="bg-neutral-900 border border-neutral-800">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <InfoBox label="METHOD" value={job.parsed_request?.method || "—"} />
          ...
          <section className="bg-neutral-900 border border-neutral-800">
            <div className="px-4 py-3 border-b border-neutral-800 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-neutral-600 mt-1">
                  Showing page {currentPage} of {meta?.totalPages || 1}
                </p>
              </div>

              <span className="text-[9px] font-mono text-neutral-500 bg-neutral-950 border border-neutral-800 px-2 py-1">
                {meta?.total ?? executions.length} TOTAL LOGS
              </span>
            </div>

            <div className="p-4">
              {executions.length === 0 ? (
                <div className="border border-neutral-800 border-dashed bg-neutral-950/50 py-16 text-center">
                ...
                <div className="space-y-2">
                  {executions.map((exec) => {
                    ...
                    return (
                      <div
                        key={exec.id}
                        className="bg-neutral-950 border border-neutral-800 hover:border-neutral-700 transition-colors"
                      >
                ...
                <div className="flex items-center justify-between pt-4 mt-4 border-t border-neutral-800">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 bg-neutral-950 border border-neutral-800 text-neutral-400 hover:text-neutral-200 disabled:opacity-30 disabled:hover:text-neutral-400 text-[10px] tracking-wider flex items-center gap-1 transition-colors"
                    >
                    ...
                    <button
                      ...
                      disabled={currentPage >= meta.totalPages || refreshing}
                      className="px-3 py-1.5 bg-neutral-950 border border-neutral-800 text-neutral-400 hover:text-neutral-200 disabled:opacity-30 disabled:hover:text-neutral-400 text-[10px] tracking-wider flex items-center gap-1 transition-colors"
                    >
                      NEXT
            ...
            {latestExecution ? (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono text-neutral-400">
                    {latestExecution.total_duration}ms
                  </span>
                </div>
